...
date: 2026-09-13T21:30:00+09:00
draft: false
categories: ["openclaw"]
...
def log_claude_call(cron_name, query_summary, model, tokens_used):
    ...
    log_entry = {
        ...
        "cron_name": cron_name,
        "query_summary": query_summary[:100],  # 처음 100자만
        "model": model,
        "tokens_used": tokens_used,
        "api_key_last_4": os.environ.get("ANTHROPIC_API_KEY", "unknown")[-4:],  # 마지막 4자만
...
# 크론 호출 직후에 호출:
# log_claude_call("ai-ml-trends-scanner", "검색어: frontier AI models", "claude-opus-4", 1523)
```
...
# 한 키로 동시 호출
echo "[체크] 1분 내 3+ 호출 (비정상 신호):"
cat "$LOGFILE" | tail -500 | jq -r '.timestamp' | sort | uniq -c | awk '$1 >= 3 {print}'
```
...
- [x] Key-MD 생성 (생성날짜 기록: 2026-09-13)
- [x] Key-Coding 생성
- [x] Key-Scanner 생성
- [x] Key-Blog 생성
...
def claude_call_with_approval_gate(query, risk_level="normal"):
    ...
    if is_high_risk or risk_level == "high":
        ...
        # 실제 환경에서는 Discord/Slack 메시지 또는 파일 시스템 폴링
        # 예: /tmp/approve-<job-id> 파일이 생기면 승인된 것으로 간주
        # 여기서는 간단히 로깅만 함
        log_approval_required(query)
        return None  # 승인 없으면 중단
